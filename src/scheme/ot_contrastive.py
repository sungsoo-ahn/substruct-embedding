...
class OptimalTransportContrastiveScheme:
    ...
    def compute_graph_simval(self, sim_mat, device):
        cost_mat = -sim_mat
        cost_mat_detach = cost_mat.detach().cpu().numpy()
        ot_mat = ot.emd(
            a=np.ones(cost_mat_detach.shape[0]) / cost_mat_detach.shape[0], 
            b=np.ones(cost_mat_detach.shape[1]) / cost_mat_detach.shape[1], 
        ...
        ot_sim = torch.sum(ot_mat_attached * sim_mat)
        return ot_sim
    ...
    def train_step(self, batch, models, optim, device):
        ...
        node_simmat_list = torch.matmul(emb, emb.T)
        
        split_sections = batch.batch_num_nodes0.tolist() + batch.batch_num_nodes1.tolist()
        node_simmat_list = torch.split(node_simmat_list, split_sections, dim=0)
        node_simmat_list = [
            torch.split(tsr, split_sections, dim=1) for tsr in node_simmat_list
            ]
        
        graph_simmat_list = [] 
        for idx0 in range(len(node_simmat_list)):
            simval_list = []
            for idx1 in range(len(node_simmat_list[0])):
                simval = self.compute_graph_simval(node_simmat_list[idx0][idx1], device)                                
                simval_list.append(simval)
            
            graph_simmat_list.append(torch.stack(simval_list, dim=0))

        similarity_matrix = torch.stack(graph_simmat_list, dim=0)

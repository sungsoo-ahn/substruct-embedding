...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == "zinc_standard_agent":
            ...
            smiles_list, rdkit_mol_objs, folds, labels = _load_chembl_with_labels_dataset(
...
def merge_dataset_objs(dataset_1, dataset_2):
    ...
    for d in dataset_1:
        ...
        data_list.append(Data(x=d.x, edge_index=d.edge_index, edge_attr=d.edge_attr, y=new_y))
...
class MoleculeFingerprintDataset(data.Dataset):
    ...
    def _process(self):
        ...
        with open(os.path.join(processed_dir, "fingerprint_data_processed.pkl"), "wb") as f:
    ...
    def _load(self):
        ...
        if "fingerprint_data_processed.pkl" in file_name_list:
            with open(os.path.join(processed_dir, "fingerprint_data_processed.pkl"), "rb") as f:
...
def _load_bbbp_dataset(input_path):
    ...
    preprocessed_smiles_list = [
        AllChem.MolToSmiles(m) if m != None else None for m in preprocessed_rdkit_mol_objs_list
    ]
...
def _load_chembl_with_labels_dataset(root_path):
    ...
    smiles_list = [
        AllChem.MolToSmiles(m) if m != None else None for m in preprocessed_rdkitArr
